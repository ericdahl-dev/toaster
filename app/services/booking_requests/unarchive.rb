# frozen_string_literal: true

module BookingRequests
  class Unarchive
    def self.call(booking_request:, metadata: {})
      new(booking_request: booking_request, metadata: metadata).call
    end

    def initialize(booking_request:, metadata: {})
      @booking_request = booking_request
      @metadata = metadata
    end

    def call
      return booking_request unless booking_request.archived_at?

      booking_request.update!(archived_at: nil)

      source = metadata[:source].presence || "manual"
      log_payload = metadata.except(:distinct_id).merge(source: source).stringify_keys

      EventLog.create!(
        account: booking_request.account,
        event_type: "booking_request.unarchived",
        subject_type: "BookingRequest",
        subject_id: booking_request.id,
        payload: log_payload
      )

      capture_telemetry("booking_request_unarchived", source: source)

      booking_request
    end

    private

    attr_reader :booking_request, :metadata

    def capture_telemetry(event, source:)
      distinct_id = metadata[:distinct_id] || "account_#{booking_request.account_id}"
      Telemetry.capture(
        distinct_id: distinct_id,
        event: event,
        properties: {
          booking_request_id: booking_request.id,
          account_id: booking_request.account_id,
          source: source
        }
      )
    end
  end
end
