...
async function requireToasterSession(): Promise<{ accountId: string; userId: number }> {
  const res = await serverFetchBackend('/auth/me');
  if (!res.ok) {
    redirect('/login?returnTo=/settings/users');
  }
  const me = (await res.json()) as { id: number; account: { id: number } };
  return { accountId: String(me.account.id), userId: me.id };
}

export default async function UsersPage() {
  const { accountId, userId } = await requireToasterSession();
  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-6 py-16 text-zinc-950 dark:bg-zinc-900 dark:text-zinc-50">
      <div className="mx-auto max-w-lg">
        ...
        <UsersClient accountId={accountId} apiBaseUrl={apiBaseUrl} currentUserId={userId} />
      </div>
    </main>
  );
}
