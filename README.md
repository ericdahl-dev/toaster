...
| `OPS_AUTH_TOKEN` | Prod | `dev-ops-token` | Bearer token for all ops endpoints including the HTML dashboard (`X-Ops-Token` header). JSON data endpoints also require `account_id` to scope data to one tenant. |
